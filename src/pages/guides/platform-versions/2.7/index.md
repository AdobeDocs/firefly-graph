---
title: Platform 2.7 Libraries - Firefly Graph
description: Libraries and import specifiers available to plugins targeting Graph platform version 2.7.
---

# Platform 2.7 — Available Libraries

This page lists every library a plugin can import when it targets Graph platform **2.7**. To target this release, set `platformVersion` to `{ "major": 2, "minor": 7 }` in the plugin's manifest. Each library below is provided by the platform at runtime, so you import it directly — there's no need to add it to your project's dependencies. Versions are the ranges guaranteed available in this release.
