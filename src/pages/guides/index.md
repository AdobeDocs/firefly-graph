...
* **Graph CLI** — the build, dev, and publish tool for plugins. Download it from the [Adobe Developer Console](https://developer.adobe.com/console) (see [Creating Plugins](platform-version-2/creating-plugins/index.md) for setup)
* **Adobe IMS credentials** — required for submitting plugins and accessing the plugin registry

## Guide Contents

New here? Start with [Core Concepts](core-concepts/index.md), then [Creating Plugins](platform-version-2/creating-plugins/index.md), and follow the guide in order.
...
* **[Developing Widgets](platform-version-2/developing-widgets/index.md)** — how to build Lit web components that view and edit datatype values. Covers the reactive signal API, display-only vs. editable widget patterns, multi-field widgets, Spectrum Web Components, and the difference between port widgets and node body widgets.
* **[Developing Nodes](platform-version-2/developing-nodes/index.md)** — how to build computational nodes with typed input and output ports. Covers the `process` function, port configuration, persistent node scope, error handling, widget binding (both port-level and node body), the common node patterns (input, preview, output, processing, conversion, composition), external API access with `fetchSources`, and file structure.
* **[Developing Utilities](platform-version-2/developing-utilities/index.md)** — how to build utility plugins that share reusable code across plugins. Covers the `createUtilityPlugin()` factory, multi-file utility structure, the `assets.exported` manifest format, and the web worker factory pattern using `new URL()`.
* **[Platform Library Reference](platform-versions/index.md)** — exactly which libraries and versions each platform release provides — Lit, Spectrum Web Components, the `@graph/*` packages, and the import paths available from each. One page per platform release; the index always points at the current release.

See also the [CLI Reference](cli-reference/index.md) for all `graph` commands: `login`, `install`, `build`, `lint`, `format`, `dev`, and `submit`.
