---
title: Platform 2.9 Libraries - Firefly Graph
description: Libraries and import specifiers available to plugins targeting Graph platform version 2.9.
---

# Platform 2.9 — Available Libraries

This page lists every library a plugin can import when it targets Graph platform **2.9**. To target this release, set `platformVersion` to `{ "major": 2, "minor": 9 }` in the plugin's manifest. Each library below is provided by the platform at runtime, so you import it directly — there's no need to add it to your project's dependencies. Versions are the ranges guaranteed available in this release.
